...
import { CREATIVE_ROUTINES } from '../constants';
...
const CreativePage: React.FC = () => {
  return (
    <div>
      <PageNavigation />
      <div className="flex items-center text-left mb-10 bg-white p-6 rounded-lg shadow-md border-l-4 border-purple-500">
        <span className="text-purple-500 mr-4">{ICONS.creative}</span>
        <div>
          <h1 className="text-4xl font-bold text-brand-dark-blue">Creative</h1>
          <p className="text-md text-gray-600">Fostering imagination and original ideas.</p>
        </div>
      </div>
      {CREATIVE_ROUTINES.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {CREATIVE_ROUTINES.map((routine, index) => (
        ...
        <div className="text-center bg-white p-10 rounded-lg shadow-md">
            ...
            <p className="text-brand-text mt-2">Check back later for routines focused on creativity.</p>
        </div>
      )}
    </div>
  );
