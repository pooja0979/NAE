...
import { COLLABORATIVE_ROUTINES } from '../constants';
...
const CollaborativePage: React.FC = () => {
  return (
    <div>
      <PageNavigation />
      <div className="flex items-center text-left mb-10 bg-white p-6 rounded-lg shadow-md border-l-4 border-blue-500">
        <span className="text-blue-500 mr-4">{ICONS.collaborative}</span>
        <div>
          <h1 className="text-4xl font-bold text-brand-dark-blue">Collaborative</h1>
          <p className="text-md text-gray-600">Working together to achieve common goals.</p>
        </div>
      </div>
      {COLLABORATIVE_ROUTINES.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {COLLABORATIVE_ROUTINES.map((routine, index) => (
        ...
        <div className="text-center bg-white p-10 rounded-lg shadow-md">
            ...
            <p className="text-brand-text mt-2">Check back later for routines focused on collaboration.</p>
        </div>
      )}
    </div>
  );
